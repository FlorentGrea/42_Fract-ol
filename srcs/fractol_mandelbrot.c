...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/03/14 22:22:17 by fgrea             #+#    #+#             */
/*   Updated: 2017/03/25 19:32:15 by fgrea            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_pxl				*fractol_mandelbrot_variables(t_pxl *pxl, int i)
{
	...
	if (i == 1)
	{
		pxl->c_i = (pxl->x / pxl->zoom_x) + pxl->x1;
		pxl->c_r = (pxl->y / pxl->zoom_y) + pxl->y1;
		pxl->z_r = pxl->tz_r;
		pxl->z_i = pxl->tz_i;
		pxl->i = 0;
	}
...
static void			*fractol_algorythm2(void *d)
{
	t_pxl			*i;
	double			tmp;
...
t_pxl				*fractol_mandelbrot(t_pxl *pxl)
{
	...
	while (++p <= 7)
	{
		...
		pthread_create(&c[p], NULL, fractol_algorythm2, &i[p]);
	}
	while (p-- >= 0)
